...
shinyServer(function(input, output, session) {
  ...
  results <- eventReactive(input$run, {
    ...
    return(list(results = results, vif = vif_values))
  })
  ...
  output$vifTable <- renderTable({
    req(results())
    vif_data <- as.data.frame(results()$vif)
    vif_data$term <- rownames(vif_data)
    vif_data <- vif_data[, c("term", "GVIF")]
    colnames(vif_data) <- c("Variabile", "VIF")
    vif_data
  })
  ...
  generateBoxPlot <- reactive({
    ...
    if (input$jitter) {
      p <- p + geom_jitter(width = 0.2, size = 1.5, alpha = 0.5, shape = 21, color = "black")
    }
